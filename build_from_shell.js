...
let sidebar = shellHtml.match(/<aside class="docs-sidebar" id="docsSidebar">([\\s\\S]*?)<\\/aside>/)[1];

// Remove sidebar section titles
...
// Remove empty sections / normalize
...
function generateContent(mod) {
    if (mod.id === 'email') {
        ...
        let content = shellHtml.match(/<div class="content-body">([\\s\\S]*?)<\\/div>\\s*<!-- Prev \\/ Next -->/);
        if (content) return content[1];
    }
    
    // For others, generate placeholder structure matching prompt.md
    return \`
        <h2 id="overview">Overview</h2>
        <p>This is the \${mod.title}. It provides essential functionality for your WordPress site. Please configure it according to your needs.</p>
        
        <h2 id="requirements">Requirements</h2>
        <ul>
            <li>WordPress 5.8 or higher</li>
            <li>PHP 7.4 or higher</li>
            <li>Ofast Toolkit plugin activated</li>
            <li>Module toggled <strong>ON</strong> in the dashboard</li>
        </ul>
        ...
        <div class="steps">
            <div class="step">
                ...
                <div class="step-content">
                    ...
                    <p>Go to <strong>Ofast Toolkit → Dashboard</strong> and toggle to <strong>ON</strong>.</p>
                </div>
            </div>
            <div class="step">
                ...
                <div class="step-content">
                    ...
                    <p>Go to the module settings page to adjust configuration.</p>
                </div>
            </div>
        </div>
        ...
        <table class="settings-table">
            ...
            <tbody>
                <tr>
                    <td><span class="setting-name">Enable Feature</span></td>
                    <td>Turns the main functionality on or off.</td>
                    <td>Disabled</td>
                </tr>
            </tbody>
        </table>
        ...
        <p>The module hooks into core WordPress functions to provide seamless integration without modifying core files.</p>
        ...
        <div class="faq-item">
            ...
            <div class="faq-answer"><p>Check the module tags above to see if it is part of the Free or Pro version.</p></div>
        </div>
    \`;
}
...
modules.forEach(m => {
    ...
    // 1. Replace Sidebar
    let currentSidebar = sidebar.replace('href="/docs/' + m.id + '.html" class="sidebar-link"', 'href="/docs/' + m.id + '.html" class="sidebar-link active"');
    html = html.replace(/<aside class="docs-sidebar" id="docsSidebar">[\\s\\S]*?<\\/aside>/, '<aside class="docs-sidebar" id="docsSidebar">' + currentSidebar + '</aside>');
    ...
    \`;
    html = html.replace(/<nav class="breadcrumb">[\\s\\S]*?<\\/nav>/, breadcrumb);
    ...
    let moduleHeader = \`
            <div class="module-header">
                ...
                <p class="module-description">Learn how to configure and use the \${m.title}.</p>
            </div>
    \`;
    html = html.replace(/<div class="module-header">[\\s\\S]*?<\\/div>\\s*<\\/div>/, moduleHeader); // match the end of meta div too

    // 4. Replace Content Body
    html = html.replace(/<div class="content-body">[\\s\\S]*?<\\/div>\\s*<!-- Prev \\/ Next -->/, '<div class="content-body">\\n' + generateContent(m) + '\\n</div>\\n\\n<!-- Prev / Next -->');
    ...
    \`;
    html = html.replace(/<nav class="docs-pagination">[\\s\\S]*?<\\/nav>/, pagination);

    // 6. Fix links - make sure they are relative if needed or match the href="/docs/..."
    // The shell uses href="/docs/...", let's keep it or change to relative "./"
